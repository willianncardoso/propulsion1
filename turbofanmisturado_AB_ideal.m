...
Tt7  = 2000;                    %temperaturas  |K|
pi_c = 10:1:20;                 %varredura em pi_c
PI_f = 2:1:5;                   %CONJUNTO FECHADO
pi_f = PI_f';                   %transposta 
              

j = length(pi_f);%cada linha do gr?fico CONJUNTO FECHADO
...
for i = 1:j    
    ...
    fAB         = ((cp*T0)/hpr).*(tal_lambidaAB-tal_lambida.*tal_t.*tal_M)/1000;
    %raz_4 = T9/T0
    raz_4       = (tal_lambidaAB)./(tal_r.*tal_f);
